...
class ImageDataset(Dataset):    #图片类，用于构建数据集
...
train_dataset = ImageDataset(root='./data', train=True, transform=transforms.ToTensor(), download=True)
test_dataset = ImageDataset(root='./data', train=False, transform=transforms.ToTensor(), download=True)
...
# %%
def denormalize(img):   #原始像素=归一化像素×标准差+均值
    mean = np.array([0.485, 0.456, 0.406])
    std = np.array([0.229, 0.224, 0.225])
    img = img * std[:, None, None] + mean[:, None, None]  # 反归一化
    img = np.clip(img, 0, 1)  # 限制范围在 [0,1]
    return img
